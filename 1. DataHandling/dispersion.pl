...
$unit_conv = $unit_scale{$units} / $input_unit;

if( $grflag ) {
  for( $units ) {
    if ( /eV/ )   { $unit_label = '\\f{Symbol}e\\f{} (eV)';}
    if ( /Ry/ )   { $unit_label = '\\f{Symbol}e\\f{} (Ry)';}
    if ( /Ha/ )   { $unit_label = '\\f{Symbol}e\\f{} (Ha)';}
    if ( /THz/ )  { $unit_label = '\\f{Symbol}w\\f{} (THz)';}
    if ( /cm-1/ ) { $unit_label = '\\f{Symbol}w\\f{} (cm\\S-1\\N)';}
    if ( /meV/ )  { $unit_label = '\\f{Symbol}w\\f{} (meV)';}
  }
} elsif ($gpflag ) {
  for( $units ) {
    if ( /eV/ )   { $unit_label = '{/Symbol e} (eV)';}
    if ( /Ry/ )   { $unit_label = '{/Symbol e} (Ry)';}
    if ( /Ha/ )   { $unit_label = '{/Symbol e} (Ha)';}
    if ( /THz/ )  { $unit_label = '{/Symbol w} (THz)';}
    if ( /cm-1/ ) { $unit_label = '{/Symbol w} (cm^{-1})';}
    if ( /meV/ )  { $unit_label = '{/Symbol w} (meV)';}
  }
}
